...
5. Distillation               replace with a compact trained model      ← distillation/
...
|---|---|---|
| `COMPRESSION_GUIDE.md` + pruning notebooks | Pruning | Unstructured (L1, 80%), structured (ln_structured, 30%), layer compaction, CSR storage |
| `quantization/` *(coming)* | Quantization | Post-training INT8, dynamic quantization, calibration |
| `distillation/` *(coming)* | Distillation | Teacher-student training, soft labels, temperature scaling |
